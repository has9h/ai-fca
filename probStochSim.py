...
class RejectionSampling(SamplingInferenceMethod):
    ...
    def query(self,qvar,obs={},number_samples=1000,sample_order=None):
        ...
        if sample_order is None:
            sample_order = self.bn.topological_sort()
        ...
        for i in range(number_samples):
            ...
            for nvar in sample_order:
                fac = self.bn.var2cpt[nvar]     #factor with nvar as child
        ...
        # As well as the distribtion we also include raw counts
...
class LikelihoodWeighting(SamplingInferenceMethod):
    ...
    def query(self,qvar,obs={},number_samples=1000,sample_order=None):
        ...
        tot = sum(counts.values())
        # as well as the distribition we also include the raw counts
...
bn_reportL = LikelihoodWeighting(bn_report)    # answers queries using rejection sampling
...
methods = [RC,RejectionSampling,LikelihoodWeighting,ParticleFiltering,GibbsSampling]
#plot_mult(methods,bn_report,Tamper,True,{Report:True,Smoke:False},number_samples=100, number_runs=1000)
# plot_mult(methods,bn_report,Tamper,True,{Report:True,Smoke:True},number_samples=100, number_runs=1000)
